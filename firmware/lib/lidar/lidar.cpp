...
{
  syslog(LOG_INFO, "%s lidar err %d", __FUNCTION__, err);
}
...
void initLidar(void) {
  ...
  //digitalWrite(LIDAR_PWM, LOW);

  comm.setRxBufferSize(BUFSIZE);
  comm.onReceiveError(rx_err_callback);
  comm.onReceive(rx_callback);
  comm.begin(LIDAR_BAUDRATE, SERIAL_8N1, LIDAR_RXD, -1);
  syslog(LOG_INFO, "setup lidar: ", __FUNCTION__);
};
